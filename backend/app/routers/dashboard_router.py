...
def dashboard(
    ...
    try:
        return _dashboard_impl(period=period, db=db, request=request, current_user=current_user)
...
def _dashboard_impl(period: str, db: Session, request: Request, current_user=None):
    now = utcnow()
    ...
    daily_rev_query = scope_query(
        db.query(func.coalesce(func.sum(Sale.total), 0)).filter(*valid_sales_filter, Sale.created_at >= today_start),
        Sale,
        request,
    )
    daily_revenue = daily_rev_query.scalar() or 0

    total_repairs = 0
    completed_repairs = 0
    if has_repairs:
        total_repairs = scope_query(
            db.query(func.count(RepairTicket.id)).filter(RepairTicket.is_deleted == False),  # noqa: E712
            RepairTicket,
            request,
        ).scalar() or 0
        completed_repairs = scope_query(
            db.query(func.count(RepairTicket.id)).filter(
    ...
    customers_count = scope_query(
        db.query(func.count(Customer.id)).filter(Customer.is_deleted == False),  # noqa: E712
        Customer,
        request,
    ).scalar() or 0
    ...
    low_stock_items = scope_query(low_stock_query, InventoryItem, request).limit(20).all()

    sales_query = db.query(Sale).filter(*valid_sales_filter).order_by(Sale.created_at.desc())
    recent_sales = scope_query(sales_query, Sale, request).limit(10).all()

    recent_repairs = []
    if has_repairs:
        rep_query = db.query(RepairTicket).filter(RepairTicket.is_deleted == False).order_by(RepairTicket.created_at.desc())  # noqa: E712
        recent_repairs = scope_query(rep_query, RepairTicket, request).limit(8).all()

    # Sales trend for the dashboard selector
    if period == "12m":
    ...
    period_sales_q = db.query(Sale.created_at, Sale.total).filter(*valid_sales_filter, Sale.created_at >= period_start)
    period_sales = scope_query(period_sales_q, Sale, request).all()
    ...
    product_rev_q = db.query(func.coalesce(func.sum(SaleItem.quantity * SaleItem.price), 0))\
        .join(Sale, Sale.id == SaleItem.sale_id)\
        .filter(*period_sales_filter, (SaleItem.line_type == "product") | (SaleItem.line_type.is_(None)))
    product_revenue = scope_query(product_rev_q, Sale, request).scalar() or 0

    spare_part_rev_q = db.query(func.coalesce(func.sum(SaleItem.quantity * SaleItem.price), 0))\
        .join(Sale, Sale.id == SaleItem.sale_id)\
        .filter(*period_sales_filter, SaleItem.line_type == "spare_part")
    spare_part_revenue = scope_query(spare_part_rev_q, Sale, request).scalar() or 0

    repair_revenue = 0
    if has_repairs:
        repair_rev_q = db.query(func.coalesce(func.sum(SaleItem.quantity * SaleItem.price), 0))\
            .join(Sale, Sale.id == SaleItem.sale_id)\
            .filter(
                *period_sales_filter,
                (Sale.repair_ticket_id.isnot(None)) | (SaleItem.line_type.in_(["labor", "service"])),
            )
        repair_revenue = scope_query(repair_rev_q, Sale, request).scalar() or 0
    ...
    if product_revenue == 0 and spare_part_revenue == 0 and repair_revenue == 0:
        total_period_sales_q = db.query(func.coalesce(func.sum(Sale.total), 0)).filter(*period_sales_filter)
        total_period_sales = scope_query(total_period_sales_q, Sale, request).scalar() or 0
        if total_period_sales > 0:
            product_revenue = total_period_sales

    try:
        out_bal_q = db.query(func.coalesce(func.sum(Sale.balance_due), 0)).filter(*valid_sales_filter, Sale.balance_due > 0)
        outstanding_balance = scope_query(out_bal_q, Sale, request).scalar() or 0
    except Exception:
        outstanding_balance = 0

    # Gross Profit calculation for period
    total_cost_q = db.query(func.coalesce(func.sum(SaleItem.quantity * SaleItem.cost_price), 0))\
        .join(Sale, Sale.id == SaleItem.sale_id)\
        .filter(*period_sales_filter)
    total_cost = scope_query(total_cost_q, Sale, request).scalar() or 0
    ...
    top_products_query = scope_query(top_products_q, Sale, request).limit(5).all()
    top_products = [
    ...
    payment_methods_query = scope_query(payment_methods_q, Sale, request).all()
    ...
    today_sales_q = db.query(Sale).filter(*valid_sales_filter, Sale.created_at >= today_start)
    today_sales = scope_query(today_sales_q, Sale, request).all()
    today_revenue = sum(float(s.total or 0) for s in today_sales)
    
    today_costs_q = db.query(func.coalesce(func.sum(SaleItem.quantity * SaleItem.cost_price), 0))\
        .join(Sale, Sale.id == SaleItem.sale_id)\
        .filter(*valid_sales_filter, Sale.created_at >= today_start)
    today_item_costs = scope_query(today_costs_q, Sale, request).scalar() or 0
    ...
    all_inventory_q = db.query(InventoryItem).filter(InventoryItem.is_deleted == False)  # noqa: E712
    all_inventory = scope_query(all_inventory_q, InventoryItem, request).all()
    ...
    recent_sold_item_ids_q = db.query(SaleItem.item_id)\
        .join(Sale, Sale.id == SaleItem.sale_id)\
        .filter(Sale.created_at >= ninety_days_ago, SaleItem.item_id.isnot(None))
    recent_sold_item_ids = set(r[0] for r in scope_query(recent_sold_item_ids_q, Sale, request).all())
    ...
    overdue_repairs_count = 0
    if has_repairs:
        seven_days_ago = now - timedelta(days=7)
        overdue_rep_q = db.query(func.count(RepairTicket.id)).filter(
            RepairTicket.is_deleted == False,  # noqa: E712
            RepairTicket.status.notin_([REPAIR_STATUS_COMPLETED, REPAIR_STATUS_DELIVERED]),
            RepairTicket.created_at <= seven_days_ago,
        )
        overdue_repairs_count = scope_query(overdue_rep_q, RepairTicket, request).scalar() or 0
    ...
    if has_warranty:
        try:
            ...
            warr_exp_q = db.query(func.count(WarrantyRecord.id)).filter(
                WarrantyRecord.is_deleted == False,  # noqa: E712
                WarrantyRecord.end_date >= now.date(),
                WarrantyRecord.end_date <= thirty_days_ahead.date(),
            )
            expiring_warranties_count = scope_query(warr_exp_q, WarrantyRecord, request).scalar() or 0
        except Exception:
            pass

    # Expiry Tracking Counter (Batches expiring within 30 days)
    expiring_batches_count = 0
    try:
        exp_batches_q = db.query(func.count(InventoryItem.id)).filter(
        ...
        expiring_batches_count = scope_query(exp_batches_q, InventoryItem, request).scalar() or 0
    except Exception:
        pass
    ...
    try:
        ...
        supp_q = db.query(Supplier).filter(Supplier.is_deleted == False)  # noqa: E712
        suppliers = scope_query(supp_q, Supplier, request).all()
    ...
    try:
        cred_q = db.query(func.count(func.distinct(Sale.customer_id)))\
            .filter(*valid_sales_filter, Sale.balance_due > 0, Sale.customer_id.isnot(None))
        pending_credit_customers_count = scope_query(cred_q, Sale, request).scalar() or 0
    except Exception:
    ...
    if has_repairs:
        grouped_status_q = db.query(RepairTicket.status, func.count(RepairTicket.id))\
            .filter(RepairTicket.is_deleted == False)\
            .group_by(RepairTicket.status)  # noqa: E712
        grouped_status_rows = scope_query(grouped_status_q, RepairTicket, request).all()
    ...
    # Activity Feed
    act_q = db.query(ActivityLog).order_by(ActivityLog.created_at.desc())
    logs = scope_query(act_q, ActivityLog, request).limit(10).all()
    activity_feed = []
    for l in logs:
    ...
    if spare_part_revenue > 0 or has_repairs:
        sales_breakdown.append({"name": "Spare Parts", "value": float(spare_part_revenue or 0)})
    if has_repairs:
        sales_breakdown.append({"name": "Repair Services", "value": float(repair_revenue or 0)})
